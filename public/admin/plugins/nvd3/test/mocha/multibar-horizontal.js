(function() {
  describe('NVD3', function() {
    return describe('MultiBar Horizontal Chart', function() {
      ...
      it('has correct structure', function() {
        ...
        cssClasses = ['.nv-x.nv-axis', '.nv-y.nv-axis', '.nv-barsWrap', '.nv-multibarHorizontal', '.nv-legendWrap', '.nv-controlsWrap'];
        results = [];
        for (i = 0, len = cssClasses.length; i < len; i++) {
          cssClass = cssClasses[i];
          results.push((function(cssClass) {
            return should.exist(builder.$("g.nvd3.nv-multiBarHorizontalChart " + cssClass)[0]);
          })(cssClass));
        }
        return results;
      });
