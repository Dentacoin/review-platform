...
window.addEventListener('load', function() {
    ...
    reviewSubmitedReward = function(dcn_address, user_id, review_content, submit_secret, invite_secret) {
        ...
        // submit function
        contract.submitReview(dcn_address, user_id, review_content, submit_secret, invite_secret, transactionObject, function(error, transactionHash){
            if(error) {
                console.log("There was an error transfering your Dentacoins: " + String(error));
                return;
            }

            console.log("Your token is being transfered with tx hash: " + String(transactionHash));
            console.log("Ok, pending transaction. Give it a minute and check for confirmation on <a href='https://etherscan.io/tx/" + String(transactionHash) + "' target='_blank'>Etherscan</a> ");
        });
        ...
        contract.SubmitEvent({}, function(error, result){
            ...
            console.log("Your Dentacoins have been transfered! " + String(result.transactionHash));
        });
    };
