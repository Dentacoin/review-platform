...
class SitemapController extends FrontController
{
	public function links($locale=null) {
		...
		$dentists = User::where('is_dentist', '1')->whereIn('status', ['approved','added_approved','admin_imported','added_by_clinic_approved'])->get();
		...
		$dentists = User::where('is_dentist', 1)->whereNotNull('address')->whereNotNull('country_id')->whereIn('status', ['approved','added_approved','admin_imported','added_by_clinic_approved'])->whereNotNull('city_name')->groupBy('country_id')->get()->pluck('country_id');
        $dentist_countries = Country::whereIn('id', $dentists )->get();

        $dentist_cities = User::where('is_dentist', 1)->whereNotNull('address')->whereIn('status', ['approved','added_approved','admin_imported','added_by_clinic_approved'])->whereNotNull('country_id')->whereNotNull('state_name')->whereNotNull('city_name')->groupBy('state_name')->groupBy('city_name')->get();
        
        $dentist_states = User::where('is_dentist', 1)->whereNotNull('address')->whereIn('status', ['approved','added_approved','admin_imported','added_by_clinic_approved'])->whereNotNull('country_id')->whereNotNull('state_name')->whereNotNull('city_name')->groupBy('state_name')->get();
