...
class SitemapController extends FrontController
{
	public function links($locale=null) {
        ...
        $dentist_cities = User::where('is_dentist', 1)->where('status', 'approved')->whereNotNull('country_id')->whereNotNull('city_name')->groupBy('city_name')->get();
    	...
    	foreach ($dentist_cities as $user) {
    		$links[] = getLangUrl('dentists/'.str_replace(' ', '-', strtolower($user->city_name)).'-'.$user->country->slug);
    	}
