					...
					<div class="form-inner">						
						...
						<div class="sign-in-step {!! empty($regData) ? 'active' : '' !!}" id="step-1">
							...
							<div class="modern-field alert-after" {!! session('join_clinic') && session('invited_by') ? 'style="margin-top: 20px;"' : '' !!}>
								<input type="email" name="email" id="dentist-email" class="modern-input" value="{{ !empty($regData) && !empty($regData['email']) ? $regData['email'] : old('email') }}" autocomplete="off" readonly onfocus="this.removeAttribute('readonly');">
							...
							<div class="modern-field alert-after">
								<input type="password" name="password" id="dentist-password" class="modern-input" value="{{ !empty($regData) && !empty($regData['password']) ? $regData['password'] : old('password') }}" autocomplete="off">
							...
							<div class="modern-field alert-after">
								<input type="password" name="password-repeat" id="dentist-password-repeat" class="modern-input" value="{{ !empty($regData) && !empty($regData['password']) ? $regData['password'] : old('password-repeat') }}" autocomplete="off">
						...
						<div class="sign-in-step {!! !empty($regData) && empty($regData['name']) ? 'active' : '' !!}" id="step-2">
							...
							<div class="mobile-radios modern-radios alert-after" {!! session('join_clinic') && session('invited_by') ? 'style="display: none;"' : '' !!}>
								<div class="radio-label col">
								  	<label for="mode-dentist" {!! !empty($regData) && !empty($regData['mode']) && $regData['mode']=='dentist' ? 'class="active"' : '' !!}>
										<span class="modern-radio">
											<span></span>
										</span>
								    	<input class="type-radio" type="radio" name="mode" id="mode-dentist" value="dentist" {!! (session('join_clinic') && session('invited_by')) || (!empty($regData) && !empty($regData['mode']) && $regData['mode']=='dentist') ? 'checked="checked"' : '' !!}>
								    	{!! nl2br(trans('trp.popup.popup-register.mode.dentist')) !!}
								  	</label>
								  	<span>{!! nl2br(trans('trp.popup.popup-register.mode.dentist.description')) !!}</span>
								</div>
								<div class="radio-label col">
								  	<label for="mode-clinic" {!! !empty($regData) && !empty($regData['mode']) && $regData['mode']=='clinic' ? 'class="active"' : '' !!}>
										<span class="modern-radio">
											<span></span>
										</span>
								    	<input class="type-radio" type="radio" name="mode" id="mode-clinic" value="clinic" {!! !empty($regData) && !empty($regData['mode']) && $regData['mode']=='clinic' ? 'checked="checked"' : '' !!}>
					  		...
					  		<div class="modern-field title-wrap alert-after tooltip-text fixed-tooltip" text="{!! trans('trp.popup.popup-register.title.tooltip') !!}" {!! !empty($regData) && $regData['mode']=='dentist' ? '' : 'style="display: none;"' !!}>
					  			<select name="title" id="dentist-title" class="modern-input" value="{{ !empty($regData) && $regData['title'] ? $regData['title'] : old('title') }}">
