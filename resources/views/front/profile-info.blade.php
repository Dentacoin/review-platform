                ...
                @include('front.errors')
      			...
      			{!! Form::open(array('url' => getLangUrl('profile/info'), 'method' => 'post', 'class' => 'form-horizontal')) !!}
                    ...
                    @foreach( $fields as $key => $info)
                        @if($key == 'title' && $user->is_clinic)

                        @else
                            <div class="form-group {{ $errors->has($key) ? 'has-error' : '' }}" >
                                <label class="col-md-2 control-label {{ $key=='categories' || $key=='work_hours' ? 'form-control-checkboxes' : '' }} ">
                                    @if($key == 'name' && $user->is_clinic)
                                        {{ trans('front.page.'.$current_page.'.form-clinic-name') }}
                                    @else
