...
@section('content')
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                @if($item->is_dentist)
                                    ...
                                    <div class="form-group">
                                        <label class="col-md-2 control-label"><img src="{{ url('img-trp/top-dentist-month.png') }}" style="max-width: 20px;" /> Top Dentist Month</label>
                                    ...
                                    <div class="form-group clearfix">
                                        <div class="col-md-offset-2 col-md-10">
                                            <a href="javascript:;" class="btn btn-primary btn-block btn-add-new-badge" style="margin-top: -10px;" >
                                                Аdd top dentist month badge
                                            </a>
                                        </div>
                                    </div>

                                    <div class="form-group">
                                        <label class="col-md-2 control-label"><img src="{{ url('img-trp/top-dentist-year.png') }}" style="max-width: 20px;" /> Top Dentist Year</label>
                                        <div class="col-md-10 top-list-year">
                                            @if($item->top_dentist_year )
                                                @foreach(explode(';',$item->top_dentist_year) as $badge)
                                                    <div class="input-group clearfix">
                                                        <div class="template-box clearfix">
                                                            <select name="badge-year-only[]" class="form-control badge-select">
                                                                @for($i=date('Y');$i>=2017;$i--)
                                                                    <option value="{{ $i }}" {{ explode(':', $badge)[0] == $i ? 'selected="selected"' : '' }}>{{ $i }}</option>
                                                                @endfor
                                                            </select>                                          
                                                        ...
                                                        <div class="input-group-btn">
                                                            <button class="btn btn-default btn-remove-year-badge" type="button">
                                                                <i class="glyphicon glyphicon-remove"></i>
                                                            </button>
                                                        </div>
                                                    </div>
                                    ...
                                    <div class="form-group clearfix">
                                        <div class="col-md-offset-2 col-md-10">
                                            <a href="javascript:;" class="btn btn-primary btn-block btn-add-new-year-badge" style="margin-top: -10px;" >
                                                Аdd top dentist year badge
                                            </a>
                                        </div>
                                    </div>
                            ...
                            <div class="col-md-6">
                                        ...
                                        <div class="col-md-12">
                                            @if(!empty($item->platform) && $item->platform == 'trp')
                                                ...
                                                @else
                                                    ...
                                                    @else
                                                        @if($item->is_dentist)
                                                            @if($item->invitorWithTrashed->is_clinic)
                                                                Added by clinic at {{ config('platforms')[$item->platform]['name'] }} - 
                                                            @else
                                                                Added by dentist at {{ config('platforms')[$item->platform]['name'] }} signup 
                                                            @endif
                                                        @else
                                                            Registered from {{ config('platforms')[$item->platform]['name'] }} friend invite 
                                                        @endif
                                                    @endif
                                                @endif
                                            @else
                                                Registered from {{ !empty($item->platform) ? config('platforms')[$item->platform]['name'] : '' }} friend invite 
                                            @endif
                                            &nbsp;&nbsp;<a href="{{ url('cms/users/users/edit/'.$item->invitorWithTrashed->id) }}">{{ $item->invitorWithTrashed->name }}</a>
                                        </div>
                                    </div>
                                @endif
                                ...
                                @if(!empty($item->invited_by) && !$item->is_dentist)
                                    <div class="form-group" style="text-align: right;">
                                        <div class="col-md-12">
                                            Registered from {{ !empty($item->platform) ? config('platforms')[$item->platform]['name'] : '' }} friend invite <a href="{{ url('cms/users/users/edit/'.$item->invitorWithTrashed->id) }}">{{ $item->invitorWithTrashed->name }}</a>
                                        </div>
                                    </div>
                                @endif
                ...
                <div class="panel panel-inverse">
                    ...
                    <div class="panel-body show-hide-area">
                        ...
                        @include('admin.parts.table', [
                            ...
                            'table_fields' => [
                                'created_at'        => array('format' => 'datetime','width' => '20%'),
                                'ip'                => array('template' => 'admin.parts.table-logins-user','width' => '20%'),
                                'is_vpn'            => array('format' => 'bool-new','width' => '10%'),
                                'country'           => array('label' => 'Country','width' => '20%'),
                                'device'            => array('template' => 'admin.parts.table-logins-device','width' => '10%'),
                                'platform'          => array('template' => 'admin.parts.table-platforms','width' => '20%'),
                            ],
                    ...
                    <div class="panel-body">
                        @include('admin.parts.table', [
                            ...
                            'table_fields' => [
                                ...
                                'explain_related'   => array('label' => 'Explain','width' => '10%'),
                                'ip'                => array('label' => 'IP','width' => '10%'),
    ...
    <div style="display: none;">
        ...
        <div class="input-group clearfix" id="badge-group-year-template">
            <div class="template-box clearfix">
                <select name="badge-year-only[]" class="form-control badge-select">
                    @for($i=date('Y');$i>=2017;$i--)
                        <option value="{{ $i }}" {{ date('Y') == $i ? 'selected="selected"' : '' }}>{{ $i }}</option>
